...
import { obterFasesLua } from 'src/utils/fasesLua';
...
/* Recalcula a lista sempre que o usuário muda de mês ou ano. */
watch(
  [carouselMonth, carouselYear],
  async ([mes, ano]) => {
    try {
      fasesLua.value = await obterFasesLua(ano, mes + 1);
    } catch (e) {
...
function emitirDataSelecionada(fase) {
  if (fase?.data) {
    emit('update:modelValue', fase.data);
  }
}
