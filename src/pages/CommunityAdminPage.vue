  ...
  <q-page class="community-admin-page">
    <section class="community-admin-card">
      ...
      <form v-if="!isAuthorized" class="community-admin-login" @submit.prevent="loadRegistrations">
        <q-input v-model="adminToken" outlined type="password" autocomplete="current-password" :label="t('community.adminToken')" />
        <q-btn unelevated no-caps color="primary" class="app-primary-action" type="submit" icon="login" :loading="isLoading" :disable="!adminToken.trim()" :label="t('community.adminConnect')" />
      </form>
      ...
      <template v-else>
        ...
        <section class="community-admin-notifications">
          ...
          <q-btn v-if="notificationStatus.configured" outline no-caps color="primary" icon="send" :loading="isConnectingTelegram" :label="t('community.adminTelegramConnect')" @click="connectTelegram" />
        </section>
        ...
        <div v-if="pendingRegistrations.length" class="community-admin-list">
          <article v-for="registration in pendingRegistrations" :key="registration.id">
            ...
            <div class="community-admin-actions">
              <q-btn outline no-caps color="negative" icon="close" :loading="actionId === registration.id" :label="t('community.adminReject')" @click="moderate(registration.id, 'rejected')" />
              <q-btn unelevated no-caps color="positive" icon="check" :loading="actionId === registration.id" :label="t('community.adminApprove')" @click="moderate(registration.id, 'approved')" />
            </div>
          </article>
        </div>
        ...
        <div class="community-admin-toolbar community-admin-toolbar--approved">
          ...
          <q-btn flat round icon="refresh" :loading="isLoading" :aria-label="t('community.adminRefresh')" @click="loadRegistrations({ preserveMessage: true })">
        ...
        <div v-if="approvedRegistrations.length" class="community-admin-list">
          <article v-for="registration in approvedRegistrations" :key="registration.id">
            ...
            <div class="community-admin-actions">
              <q-btn flat round icon="open_in_new" :href="getProfileUrl(registration)" target="_blank" rel="noopener noreferrer" :aria-label="t('community.memberProfile', { name: registration.publicName })">
                <q-tooltip>{{ t('community.memberProfile', { name: registration.publicName }) }}</q-tooltip>
              </q-btn>
              <q-btn outline no-caps color="primary" icon="manage_accounts" :label="t('community.adminManage')" @click="openManagement(registration)" />
            </div>
          </article>
        </div>
      ...
      <p v-if="message" class="community-admin-message" :class="{ 'community-admin-message--error': hasError }">{{ message }}</p>
    </section>
    ...
    <q-dialog v-model="managementOpen" persistent>
      <q-card class="community-management-dialog">
        <q-card-section class="community-management-dialog__header">
          ...
          <q-btn flat round dense icon="close" v-close-popup :aria-label="t('navigation.closeMenu')" />
        </q-card-section>
        <q-separator />

        <q-card-section class="community-management-dialog__body">
          <!-- A captura é recortada manualmente antes do envio e permanece na base protegida. -->
          <div class="community-management-dialog__avatar">
            <span class="community-admin-avatar community-admin-avatar--large">
              <img v-if="managedRegistration?.avatarUrl" :src="managedRegistration.avatarUrl" alt="" />
          ...
          <q-file v-model="selectedAvatarFile" outlined dense accept="image/jpeg,image/png,image/webp" max-file-size="524288" :label="t('community.adminPhotoSelect')" @rejected="showAvatarSizeError">
            <template #prepend><q-icon name="photo_camera" /></template>
          </q-file>
          <div class="community-management-dialog__row-actions">
            <q-btn outline no-caps color="primary" icon="upload" :disable="!selectedAvatarFile" :loading="actionName === 'avatar'" :label="t('community.adminPhotoUpload')" @click="uploadAvatar" />
            <q-btn v-if="managedRegistration?.avatarUrl" flat no-caps color="negative" icon="hide_image" :loading="actionName === 'avatar-remove'" :label="t('community.adminPhotoRemove')" @click="removeAvatar" />
          </div>

          <q-separator />
          <div class="community-management-dialog__fields">
            <q-input v-model.trim="editForm.publicName" outlined dense :label="t('community.nameLabel')" />
            <q-select v-model="editForm.socialNetwork" outlined dense emit-value map-options :options="socialNetworkOptions" :label="t('community.networkLabel')" />
            <q-input v-model.trim="editForm.socialProfile" outlined dense :label="t('community.profileLabel')" />
            <q-input v-model.trim="editForm.country" outlined dense maxlength="2" :label="t('community.adminCountry')" />
          </div>
          <q-btn unelevated no-caps color="primary" class="app-primary-action" icon="save" :loading="actionName === 'save'" :label="t('community.adminSave')" @click="saveRegistration" />
          ...
          <section class="community-management-dialog__ownership">
            ...
            <q-btn outline no-caps color="primary" icon="key" :loading="actionName === 'deletion-code'" :label="t(managedRegistration?.hasDeletionCode ? 'community.adminDeletionRotate' : 'community.adminDeletionCreate')" @click="generateDeletionCode" />
          </section>
          <q-btn outline no-caps color="negative" icon="delete_forever" :label="t('community.adminDelete')" @click="deleteConfirmationOpen = true" />
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="deletionCodeOpen" persistent>
      <q-card class="community-code-dialog">
        ...
        <q-card-actions align="right">
          <q-btn flat no-caps icon="content_copy" :label="t('community.adminCopy')" @click="copyDeletionLink" />
          <q-btn unelevated no-caps color="primary" :label="t('community.adminDone')" v-close-popup />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="deleteConfirmationOpen">
      <q-card class="community-confirm-dialog">
        <q-card-section>
          ...
          <p>{{ t('community.adminDeleteConfirmDescription', { name: managedRegistration?.publicName }) }}</p>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat no-caps :label="t('holidaySettings.cancel')" v-close-popup />
          <q-btn unelevated no-caps color="negative" icon="delete_forever" :loading="actionName === 'delete'" :label="t('community.adminDelete')" @click="deleteManagedRegistration" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
const editForm = reactive({ publicName: '', socialNetwork: 'instagram', socialProfile: '', country: '' });
const { t, locale } = useI18n({ useScope: 'global' });
const router = useRouter();

const pendingRegistrations = computed(() => registrations.value.filter(({ status }) => status === 'pending'));
const approvedRegistrations = computed(() => registrations.value.filter(({ status }) => status === 'approved'));
...
const telegramStatusText = computed(() => {
  ...
  if (notificationStatus.value.connected) return t('community.adminTelegramConnected', { username: notificationStatus.value.username || 'jean7rafael' });
...
async function connectTelegram() {
  ...
  try {
    ...
    const payload = await response.json();
    if (!response.ok || !payload?.telegram?.connected) throw new Error(payload?.error || 'telegram_connection_failed');
    notificationStatus.value = payload.telegram;
    showMessage(t('community.adminTelegramSuccess'));
  } catch (error) {
    showMessage(error?.message === 'telegram_chat_not_found' ? t('community.adminTelegramNotFound') : t('community.adminTelegramError'), true);
  } finally {
...
async function moderate(id, status) {
  ...
  message.value = '';
  try {
    const response = await fetch(endpoint, { method: 'PATCH', headers: authorizationHeaders(true), body: JSON.stringify({ status }) });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
async function saveRegistration() {
  if (!managedRegistration.value || !editForm.publicName.trim() || !editForm.socialProfile.trim()) return;
  actionName.value = 'save';
  try {
    const endpoint = getCommunityApiUrl(`admin/registrations/${encodeURIComponent(managedRegistration.value.id)}`);
    const response = await fetch(endpoint, {
      method: 'PUT',
      headers: authorizationHeaders(true),
      body: JSON.stringify({ publicName: editForm.publicName.trim(), socialNetwork: editForm.socialNetwork, socialProfile: editForm.socialProfile.trim(), locale: managedRegistration.value.locale || locale.value, country: editForm.country.trim().toUpperCase() }),
    });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
async function uploadAvatar() {
  ...
  try {
    const endpoint = getCommunityApiUrl(`admin/registrations/${encodeURIComponent(managedRegistration.value.id)}/avatar`);
    const response = await fetch(endpoint, { method: 'PUT', headers: { ...authorizationHeaders(), 'Content-Type': selectedAvatarFile.value.type }, body: selectedAvatarFile.value });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    selectedAvatarFile.value = null;
    await reloadManagedRegistration();
...
async function generateDeletionCode() {
  ...
  try {
    const endpoint = getCommunityApiUrl(`admin/registrations/${encodeURIComponent(managedRegistration.value.id)}/deletion-code`);
    const response = await fetch(endpoint, { method: 'POST', headers: authorizationHeaders() });
    const payload = await response.json();
    if (!response.ok || !payload?.deletionCode) throw new Error(`HTTP ${response.status}`);
    const route = router.resolve({ name: 'community-remove', query: { code: payload.deletionCode } });
...
function showAvatarSizeError() { showMessage(t('community.adminPhotoSizeError'), true); }
function showMessage(text, error = false) { message.value = text; hasError.value = error; }
...
function getProfileUrl(registration) {
  ...
  if (registration?.socialNetwork === 'instagram') return `https://www.instagram.com/${encodeURIComponent(username)}/`;
  if (registration?.socialNetwork === 'facebook') return `https://www.facebook.com/${encodeURIComponent(username)}`;
  return profile;
}

function getMemberInitial(name) { return String(name || '?').trim().charAt(0).toUpperCase() || '?'; }
...
<style scoped>
.community-admin-page { width: min(100%, 1040px); min-height: calc(100vh - 58px); display: grid; place-items: start center; margin: 0 auto; padding: 48px 20px; color: var(--app-text); }
.community-admin-card { width: 100%; padding: clamp(22px, 5vw, 42px); background: var(--app-surface); border: 1px solid var(--app-border); border-radius: 20px; box-shadow: var(--app-card-shadow); }
...
.community-admin-login { max-width: 520px; display: grid; grid-template-columns: 1fr auto; align-items: start; gap: 12px; margin: 32px auto 0; }
.community-admin-toolbar { display: flex; align-items: center; justify-content: space-between; margin-top: 32px; }
.community-admin-toolbar--approved { margin-top: 44px; padding-top: 28px; border-top: 1px solid var(--app-border); }
.community-admin-toolbar h2 { margin: 0; font-size: 18px; }
.community-admin-toolbar small { display: block; margin-top: 4px; color: var(--app-text-faint); }
.community-admin-notifications { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 14px; margin-top: 32px; padding: 16px; color: var(--app-text); background: color-mix(in srgb, #8b5cf6 8%, var(--app-surface)); border: 1px solid color-mix(in srgb, #8b5cf6 34%, var(--app-border)); border-radius: 15px; }
...
.community-admin-list article { display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 16px; border: 1px solid var(--app-border); border-radius: 15px; }
.community-admin-identity { min-width: 0; display: flex; align-items: center; gap: 12px; }
.community-admin-identity > span:not(.community-admin-avatar) { font-size: 23px; }
.community-admin-identity strong, .community-admin-identity small, .community-admin-identity time { display: block; }
...
.community-admin-actions { flex: none; display: flex; align-items: center; gap: 8px; }
.community-admin-avatar { width: 44px; height: 44px; flex: none; display: grid; place-items: center; overflow: hidden; color: white; background: linear-gradient(135deg, #2563eb, #7c3aed); border-radius: 50%; font-weight: 800; }
.community-admin-avatar img { width: 100%; height: 100%; display: block; object-fit: cover; }
.community-admin-avatar--large { width: 70px; height: 70px; font-size: 22px; }
...
.community-management-dialog, .community-code-dialog, .community-confirm-dialog { width: min(680px, 94vw); color: var(--app-text); background: var(--app-surface); border: 1px solid var(--app-border); border-radius: 18px; }
.community-management-dialog__header { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; }
.community-management-dialog__body { display: grid; gap: 18px; max-height: 75vh; overflow-y: auto; }
.community-management-dialog__avatar { display: flex; align-items: center; gap: 16px; }
.community-management-dialog__avatar p, .community-management-dialog__ownership p, .community-code-dialog p, .community-confirm-dialog p { margin: 4px 0 0; color: var(--app-text-muted); }
.community-management-dialog__row-actions { display: flex; flex-wrap: wrap; gap: 8px; }
.community-management-dialog__fields { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.community-management-dialog__ownership { display: flex; align-items: center; justify-content: space-between; gap: 18px; }
...
@media (max-width: 680px) {
  ...
  .community-admin-actions { justify-content: flex-end; flex-wrap: wrap; }
  .community-management-dialog__fields { grid-template-columns: 1fr; }
  .community-management-dialog__ownership { align-items: stretch; flex-direction: column; }
}
</style>
