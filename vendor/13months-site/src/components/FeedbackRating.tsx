...
/* ===========================================================
   IDENTIDADE ANÔNIMA E ENDPOINT DA VOTAÇÃO

   O identificador nasce no navegador e serve somente para que a
   mesma instalação atualize a própria escolha. Nenhum dado pessoal
   ou endereço IP é enviado junto com o voto.
=========================================================== */

const COMMUNITY_API_URL = String(import.meta.env.VITE_COMMUNITY_API_URL || '')
  .trim()
  .replace(/\/$/, '');
const FEEDBACK_API_URL = `${COMMUNITY_API_URL}/feedback/votes`;
...
function readOrCreateVoterId() {
  try {
    ...
    localStorage.setItem(VOTER_STORAGE_KEY, newId);
    return newId;
  } catch {
...
function SmileyFace(props: {
  ...
  return (
    ...
    >
      ...
      <circle cx={mid} cy={mid + shadowOffset} r={r} fill={props.darkerColor} />
      ...
      <g transform={`translate(0, ${dy})`}>
        ...
        <circle cx={mid - eyeSpread} cy={mid + eyeY} r={eyeR} fill="white" />
        <circle cx={mid + eyeSpread} cy={mid + eyeY} r={eyeR} fill="white" />
...
export default function FeedbackRating() {
  const [votes, setVotes] = createSignal<number[]>(new Array(options.length).fill(0));
  ...
  const voterId = readOrCreateVoterId();
  ...
  onMount(async () => {
    ...
    try {
      ...
      const res = await fetch(`${FEEDBACK_API_URL}?${query}`, {
        headers: { Accept: 'application/json' },
      });
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data.votes) && data.votes.length === options.length) {
          setVotes(data.votes);
        }
      ...
      setError('Votes could not be loaded. Please try again later.');
    } finally {
      setLoading(false);
    }
  });
  ...
  function revertOptimistic(optimistic: number[], index: number, previousVote: number | null) {
  ...
  async function handleVote(index: number) {
    ...
    if (previousVote !== null && previousVote >= 0 && optimistic[previousVote] > 0) {
    ...
    try {
      const res = await fetch(FEEDBACK_API_URL, {
        ...
        body: JSON.stringify({ index, voterId }),
      });
            ...
            >
              <For each={options}>
                {(option, i) => {
                  ...
                  return (
                    <div class="flex flex-col items-center" style={{ gap: '0.45rem' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          transform: isSelected() && !isPressed() ? 'scale(1.06)' : 'scale(1)',
                        }}
                      >
                        <div
                          style={{
                        ...
                        >
                          <SmileyFace
                            ...
                            darkerColor={option.darkerColor}
                            pressed={isPressed()}
                          />
                        </div>
                      </button>
                      ...
                      <div
                        style={{
                          ...
                          height: '6px',
                          'border-radius': '50%',
                          'background-color': isSelected() ? option.faceColor : 'transparent',
                      ...
                      <div class="flex items-center gap-2.5">
                        ...
                        <div class="flex-1 h-4.5 rounded-full bg-stone-100 dark:bg-white/5 overflow-hidden relative">
                          <div
                            ...
                            style={{
                              width: `${barWidth()}%`,
                              'background-color': option.faceColor,
